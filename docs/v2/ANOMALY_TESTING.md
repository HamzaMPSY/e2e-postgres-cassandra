...
If you added the MySQL checks to an existing local stack, recreate the MySQL data volume or apply equivalent `ALTER TABLE ... ADD CONSTRAINT` statements before expecting negative payment/refund source rejects. Mongo support-ticket validation is applied through `collMod` by `db/mongo/init.js`.
