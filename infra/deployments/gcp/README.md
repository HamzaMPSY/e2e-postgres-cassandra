...
- Use GKE-hosted Debezium instead of Datastream when Kafka topic semantics must exactly match the local demo. In that mode, start from `config/connectors/production/` and replace the `secrets` config provider with the approved GCP Secret Manager provider.
